#!/usr/bin/env python3
"""
Script para actualizar datos de ficha desde SQL Server a PostgreSQL
Ejecuta la query 1_Identificadores_y_data_operacional.sql, carga los datos,
...
CSV_OUTPUT_NAME = "1__Identificadores_y_data_operacional.csv"
...
def leer_query_sql():
    ...
    # Ruta al archivo SQL dentro del repo o variable de entorno SQL_FILE_IDENT_OP
    sql_from_env = os.getenv('SQL_FILE_IDENT_OP')
    if sql_from_env:
        query_file = Path(sql_from_env)
    else:
        query_file = REPO_ROOT / "sql/1_Identificadores_y_data_operacional.sql"
    ...
    # Limpiezas suaves por compatibilidad
    query = query.strip()
    if query.endswith(';;'):
        query = query[:-1]
...
def cargar_a_postgresql(datos, columnas):
    ...
    try:
        ...
        log('Limpiando tabla nimbi."01_identificadores_y_data_operacional"...')
...
def main():
    """Función principal"""
    log("=" * 70)
    log("ACTUALIZACIÓN DATOS DE FICHA: SQL SERVER → POSTGRESQL")
    log("=" * 70)
