#!/usr/bin/env python3
"""
Script para actualizar datos académicos desde SQL Server a PostgreSQL
Ejecuta la query 9_datos_academicos.sql y carga los datos
"""
...
# Estructura esperada: integracion_nimbi/integracion_api_crm_servicios/.env
...
def leer_query_sql():
    ...
    # Ruta al archivo SQL dentro del repo o variable de entorno SQL_FILE_ACADEMICOS
    sql_from_env = os.getenv('SQL_FILE_ACADEMICOS')
    if sql_from_env:
        query_file = Path(sql_from_env)
    else:
        query_file = REPO_ROOT / "sql/9_datos_academicos.sql"
...
def cargar_a_postgresql(datos, columnas):
    ...
    try:
        ...
        log('Limpiando tabla nimbi."09_datos_academicos"...')
        cursor.execute('TRUNCATE TABLE nimbi."09_datos_academicos" RESTART IDENTITY CASCADE;')
        conn.commit()
        ...
        # Query de inserción
        insert_query = f"""
        INSERT INTO nimbi."09_datos_academicos" (
            {', '.join(columnas)}
        ) VALUES %s
        """
        ...
        # Verificar total
        cursor.execute('SELECT COUNT(*) FROM nimbi."09_datos_academicos";')
...
def main():
    """Función principal"""
    log("=" * 70)
    log("ACTUALIZACIÓN DATOS ACADÉMICOS: SQL SERVER → POSTGRESQL")
    log("=" * 70)
