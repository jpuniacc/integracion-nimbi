#!/usr/bin/env python3
"""
Script para actualizar informe de finanzas desde SQL Server a PostgreSQL
Ejecuta la query 13_informacion_finanzas.sql y carga los datos
"""
...
def leer_query_sql():
    ...
    # Ruta dentro del repo o desde variable de entorno SQL_FILE_FINANZAS
    sql_from_env = os.getenv('SQL_FILE_FINANZAS')
    if sql_from_env:
        query_file = Path(sql_from_env)
    else:
        query_file = REPO_ROOT / "sql/13_informacion_finanzas.sql"
...
def extraer_datos_sqlserver(query):
    ...
    try:
        ...
        # Obtener nombres de columnas (convertir a minúsculas y normalizar caracteres especiales)
        # Mapeo explícito de columnas con caracteres especiales
        column_mapping = {
            'mat_primer_año': 'mat_primer_anio'
        }
        columnas = []
        ...
        log(f"Primeras columnas: {columnas[:5]}")
...
def cargar_a_postgresql(datos, columnas):
    ...
    try:
        ...
        log('Limpiando tabla nimbi."13_informacion_finanzas"...')
        cursor.execute('TRUNCATE TABLE nimbi."13_informacion_finanzas" RESTART IDENTITY CASCADE;')
        conn.commit()
        ...
        # Query de inserción
        insert_query = f"""
        INSERT INTO nimbi."13_informacion_finanzas" (
            {', '.join(columnas)}
        ) VALUES %s
        """
        ...
        # Verificar total
        cursor.execute('SELECT COUNT(*) FROM nimbi."13_informacion_finanzas";')
...
def main():
    """Función principal"""
    log("=" * 70)
    log("ACTUALIZACIÓN INFORMACIÓN FINANZAS: SQL SERVER → POSTGRESQL")
    log("=" * 70)
