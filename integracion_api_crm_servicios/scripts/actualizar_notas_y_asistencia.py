#!/usr/bin/env python3
"""
Script para actualizar notas y asistencia desde SQL Server a PostgreSQL
Ejecuta la query 4_notas_y_asistencia.sql, carga los datos,
...
CSV_OUTPUT_NAME = "4__Notas_y_asistencia.csv"
...
def leer_query_sql():
    ...
    # Ruta al archivo SQL dentro del repo o variable de entorno SQL_FILE_NOTAS
    sql_from_env = os.getenv('SQL_FILE_NOTAS')
    if sql_from_env:
        query_file = Path(sql_from_env)
    else:
        query_file = REPO_ROOT / "sql/4_notas_y_asistencia.sql"
...
def cargar_a_postgresql(datos, columnas):
    ...
    try:
        ...
        log('Limpiando tabla nimbi."04_notas_y_asistencias"...')
        cursor.execute('TRUNCATE TABLE nimbi."04_notas_y_asistencias" RESTART IDENTITY CASCADE;')
        conn.commit()
        ...
        # Query de inserción
        insert_query = f"""
        INSERT INTO nimbi."04_notas_y_asistencias" (
            {', '.join(columnas)}
        ) VALUES %s
        """
        ...
        # Verificar total
        cursor.execute('SELECT COUNT(*) FROM nimbi."04_notas_y_asistencias";')
...
def generar_csv_nimbi(conn, columnas):
    ...
    try:
        ...
        FROM nimbi."04_notas_y_asistencias"
        ORDER BY anio, periodo, codcli
        """
...
def main():
    """Función principal"""
    log("=" * 70)
    log("ACTUALIZACIÓN NOTAS Y ASISTENCIA: SQL SERVER → POSTGRESQL")
    log("=" * 70)
